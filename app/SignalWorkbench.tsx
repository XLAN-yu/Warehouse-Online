    ...
    {toolMode === "transform" ? <section className="workspace" aria-label="傅里叶变换工作台">
      ...
      <div className="plot-grid">
        <article className="plot-panel">
          ...
          {direction === "forward" ? <SignalPlot id="time-main" label="时域输入信号" points={signal} mode={domainMode} zoom={timeZoom} accent="cyan" markerLabel="x" onPointerDown={propertyId === "timeShift" && transformKind === "fourier" ? (event) => { event.currentTarget.setPointerCapture(event.pointerId); setDraggingTime(true); updateTimeShift(event); } : undefined} onPointerMove={draggingTime ? updateTimeShift : undefined} onPointerUp={(event) => { if (draggingTime && event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId); setDraggingTime(false); }} /> : <FrequencyPlot points={displayInverseSpectrum} zoom={frequencyZoom} label="逆变换频域输入" />}
