...
async function createStocktake(user: CurrentUser, payload: Record<string, unknown>) {
  ...
  for (const product of productsResult.results) {
    ...
    statements.push(
      db
        .prepare("UPDATE products SET current_stock = ?, average_cost_cents = ?, status = CASE WHEN id IN (SELECT product_id FROM document_items WHERE document_id = ?) AND status = 'pending_stocktake' THEN 'normal' ELSE status END, updated_at = ? WHERE id = ?")
        .bind(state.quantity, state.averageCost, document.id, timestamp, product.id),
    );
  }
  for (const update of replay.itemUpdates) {
    statements.push(
